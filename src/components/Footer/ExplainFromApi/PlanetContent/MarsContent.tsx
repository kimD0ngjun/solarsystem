...
import {
  ...
  phobosClick,
  deimosClick,
} from "../../../../redux/contentClick/MarsContentSlice";
...
const MarsItem = styled.img<ClickMars>`
  ...
  border-radius: 50%;
  z-index: 4;
  box-shadow: ${({ clickMars }) =>
    clickMars ? "0 0 30rem 0 violet, inset 0 0 20rem 0 violet" : null};
...
const PhobosItem = styled.img<ClcikPhobos>`
  ...
  position: absolute;
  z-index: 2;
  box-shadow: ${({ clickPhobos }) =>
    clickPhobos ? "0 0 30rem 0 violet, inset 0 0 30rem 0 violet" : null};
`;
...
const DeimosItem = styled.img<ClcikDeimos>`
  ...
  position: absolute;
  z-index: 1;
  box-shadow: ${({ clickDeimos }) =>
    clickDeimos ? "0 0 30rem 0 violet, inset 0 0 10rem 0 violet" : null};
`;

const MarsContent: React.FC<PlanetProps> = (props) => {
  const { speed, radianCount, isPaused } = props;
  const dispatch: AppDispatch = useAppDispatch();
  const clickMars = useAppSelector(
    (state: RootState) => state.marsContent.Mars
  );
  const clickPhobos = useAppSelector(
    (state: RootState) => state.marsContent.Phobos
  );
  const clickDeimos = useAppSelector(
    (state: RootState) => state.marsContent.Deimos
  );
  ...
  return (
    <>
      ...
      >
        <PhobosItem
          src="/assets/PlanetImg/Phobos.svg"
          alt="phobos"
          clickPhobos={clickPhobos}
        />
      </PhobosOrbit>
      <DeimosOrbit
        speed={speed}
      ...
      >
        <DeimosItem
          src="/assets/PlanetImg/Deimos.svg"
          alt="deimos"
          clickDeimos={clickDeimos}
        />
      </DeimosOrbit>
    </>
  );
