...
import {
  uranusClick,
  titaniaClick,
} from "../../../../redux/contentClick/UranusContentSlice";
...
const UranusItem = styled.img<ClickUranus>`
  ...
  border-radius: 50%;
  z-index: 3;
  box-shadow: ${({ clickUranus }) =>
    clickUranus ? "0 0 30rem 0 violet, inset 0 0 5rem 0 violet" : null};
...
const TitaniaItem = styled.img<ClcikTitania>`
  ...
  position: absolute;
  z-index: 2;
  box-shadow: ${({ clickTitania }) =>
    clickTitania ? "0 0 10rem 0 violet, inset 0 0 10rem 0 violet" : null};
...
const UranusContent: React.FC<PlanetProps> = (props) => {
  ...
  const clickUranus = useAppSelector(
    (state: RootState) => state.uranusContent.Uranus
  );
  const clickTitania = useAppSelector(
    (state: RootState) => state.uranusContent.Titania
  );
  ...
  return (
    <>
      <UranusItem
        src="/assets/PlanetImg/Uranus.svg"
        alt="Uranus"
        clickUranus={clickUranus}
        onClick={handleClickUranus}
      />
      <TitaniaOrbit
        speed={speed}
        isPaused={isPaused}
      ...
      >
        <TitaniaItem
          src="/assets/PlanetImg/Titania.svg"
          alt="Titania"
          clickTitania={clickTitania}
        />
      </TitaniaOrbit>
    </>
  );
